...
class OptimizedRemote:
        ...
        try:
            ...
            if config:
                return {
                    ...
                    'subject_prefix': DEPLOYMENT_NAMESPACE  # Use hardcoded namespace
                }
        except Exception as e:
                ...
                try:
                    ...
                    if msg.data:
                        try:
                            ...
                            print(f"Invalid JSON from hub: {msg.data}")
                            print(f"JSON error: {e}")
                    else:
                        print("Empty response from hub")
    ...
    def request_hub_shutdown(self):
        """Request hub shutdown"""
        print(f"Hub shutdown requested by {self.remote_id}")
        ...
        try:
            ...
            if success:
                print("Hub shutdown request sent successfully")
                return True, "Hub shutdown initiated"
            else:
...
@app.route('/api/shutdown-hub', methods=['POST'])
def shutdown_hub():
    """Request hub shutdown"""
    if not remote_app.is_paired:
        return jsonify({'success': False, 'message': 'Not paired with hub'})
    
    success, message = remote_app.request_hub_shutdown()
    return jsonify({'success': success, 'message': message})
